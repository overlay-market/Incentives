...
contract PowerCardNFTs is IPowerCardNFTs, AccessControlEnumerable, ERC1155 {
  ...
  modifier onlyMinter() {
    if(!hasRole(keccak256("MINTER"), msg.sender)) revert PowerCardNFTs_NotMinter();
    _;
  }

  modifier onlyBurner() {
    if(!hasRole(keccak256("BURNER"), msg.sender)) revert PowerCardNFTs_NotBurner();
    _;
  }

  modifier onlyAdmin() {
    if(!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert PowerCardNFTs_NotBurner();
    _;
  }

  function mint(address _to, uint _id, uint _amount) external override onlyMinter {
    totalSupply[_id] += _amount;
    _mint(_to, _id, _amount, "");
  }

  function mintBatch(address _to, uint[] memory _ids, uint[] memory _amounts) external onlyMinter {
  ...
  function burnBatch(uint[] memory _ids, uint[] memory _amounts) external onlyBurner {
    for (uint256 i = 0; i < _ids.length; i++) {
      totalSupply[_ids[i]] -= _amounts[i];
    }
  ...
  function redeemPowerCardNFT(uint256 _burnId, uint256 _burnAmount, uint256 _mintId) external {
    // Todo 
